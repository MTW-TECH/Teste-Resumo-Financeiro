...
function index({
  ...
  const [extraActionPerformed, setExtraActionPerformed] = useState(false);
  ...
  const onLocalExtraAction = () => {
    onExtraAction();
    setExtraActionPerformed(true);
  };

  useEffect(() => {
    if (extraActionPerformed && extraActionResponse) {
      if (extraActionResponse !== 'loading') {
        setExtraActionPerformed(false);
      }
    }
  }, [extraActionResponse]);

  useEffect(() => {
    if (submitPerformed && submitResponse) {
  ...
  const modalFrameContainer = {
    ...
    minHeight: modalHeight ? modalHeight : 'auto'
  };
  ...
  const modalFrameBodyWrapper = {
    visibility: 'visible',
    overflowY: 'auto',
    minHeight: childrenHeight ? childrenHeight : 'auto'
  };
  ...
  const modalFrameEmptyChildrenWrapper = {
    height: childrenHeight ? childrenHeight : '360px',
    overflow: 'auto'
  };
              ...
              {secondSection ? (
                ...
                >
                  <SectionTitle
                    ...
                    title={secondSection}
                  />
                  <div style={modalFrameChildrenWrapper}>{children}</div>
                </Box>
              ) : null}
              {thirdSection ? (
                <Box sx={{ padding: '0px 0px 26px 0px' }}>
                  <SectionTitle size={'medium'} title={thirdSection} />
                </Box>
              ) : null}
              {fourthSection ? (
                <Box sx={{ padding: '15px 0px 26px 0px' }}>
                  <SectionTitle size={'medium'} title={fourthSection} />
                </Box>
              ) : null}
            </>
          ) : modalType === 'empty' ? (
          ...
          <div style={modalFrameButtonsInnerWrapper}>
            ...
            {extraActionTitle && (
              <Button
                ...
                outline
                sx={{ height: '40px' }}
                onClick={onLocalExtraAction}
              >
                {extraActionPerformed ? (
                  <FetchingAnimation size={'30px'} color={'#A646DC'} />
                ) : (
                  extraActionTitle
                )}
              </Button>
            ...
            <div style={modalFrameButtonsWrapper}>
              ...
              <div style={modalFrameButtonsInnerWrapper}>
                ...
                <Button
                  ...
                  text="Enviar"
                  primary
                  onClick={!submitPerformed ? twoStepSubmit : null}
                >
                  {submitPerformed ? (
                    <FetchingAnimation size={'30px'} color={'#ffffff'} />
                  ) : (
                    submitTitle
                  )}
                </Button>
